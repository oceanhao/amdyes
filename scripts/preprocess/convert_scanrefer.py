...
from utils import _9dof_from_world_to_camera, get_images_with_gt_instance, get_images
...
def embodeidscan_bbox_tranfrom(bbox, scale=1.0):
...
def scanrefer_bbox_transform(box, scale=1.0):
    center = np.array(box[:3]) * scale
    size = np.array(box[3:6]) * scale
    rot = R.from_matrix(np.eye(3))
    euler_zxy = rot.as_euler("zxy")
    bbox = center.tolist() + size.tolist() + euler_zxy.tolist()
    return bbox

def find_instance(scan, box):
    instances = scan['instances']
    max_iou = 0
    instance_id = -1
    for i, instance in enumerate(instances):
        bbox1 = embodeidscan_bbox_tranfrom(instance["bbox_3d"], 100)
        bbox2 = scanrefer_bbox_transform(box, 100)
        iou = EulerDepthInstance3DBoxes.overlaps(
            EulerDepthInstance3DBoxes(torch.tensor([bbox1])),
            EulerDepthInstance3DBoxes(torch.tensor([bbox2]))
        )
        if max_iou < iou:
            max_iou = iou
            instance_id = i
...
def get_output(item, scan, desc, answer, images, box, split, object_json=None):
...
Text: {desc}
Output a JSON dictionary with the frame index in "frame" and its 3D bounding box in "box_3d" in the frame's coordinates.
"""
...
def main(data, args):
    ...
    for i, item in enumerate(tqdm(data)):
        ...
        if split == "train":
            gt_instance_id, iou = find_instance(scan, box)
            if iou < 0.5:
                missing_num += 1
            ...
            for _ in range(2):
                images, frame_id = get_images_with_gt_instance(scan, args.nframes, gt_instance_id)
                if frame_id == -1:
                    missing_num += 1
                ...
                new_bbox_3d = _9dof_from_world_to_camera(box, extrinsic)
                object_json = {
                    "frame": frame_id,
                    "bbox_3d": [round(x, 2) for i, x in enumerate(new_bbox_3d)]
                }
                answer = f"```json\n\t{json.dumps(object_json)}\n```"
                output = get_output(item, scan, desc, answer, images, box, split=split, object_json=object_json)
                all_data.append(output)
        else:
            images = get_images(scan['images'], args.nframes)
            answer = ""
            output = get_output(item, scan, desc, answer, images, box, split=split)
            all_data.append(output)
